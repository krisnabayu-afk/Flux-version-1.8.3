...
export const CustomToolbar = ({ date, onNavigate, onView, view, onZoom, zoomLevel }) => {
  ...
  return (
    <div className="rbc-toolbar flex flex-col md:flex-row items-center justify-between gap-4 mb-4 !h-auto bg-slate-900/40 p-2 rounded-xl border border-slate-700/50">
      {/* Navigation Group */}
      <div className="flex items-center gap-2 w-full md:w-auto justify-center md:justify-start">
        ...
        <div className="relative">
          <button
            ...
            className="font-bold text-sm md:text-base hover:bg-slate-800 transition-colors cursor-pointer bg-slate-800/50 border border-slate-700/50 rounded-lg px-4 py-2 text-foreground ml-2"
          >
            {moment(date).format('MMMM YYYY')}
          </button>
          {showDatePicker && (
            <div className="absolute top-full left-0 mt-2 bg-slate-900 border border-slate-700 rounded-lg shadow-xl p-4 z-50 min-w-[300px]">
      ...
      <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-end">
        ...
        {(view === 'week' || view === 'day') && (
          <div className="flex items-center bg-slate-800/50 border border-slate-700/50 rounded-lg p-1 h-[38px]">
            <button
              type="button"
              onClick={handleZoomOut}
              disabled={zoomLevel <= 40}
              title="Zoom Out"
              className="p-1.5 hover:bg-slate-700 rounded-md transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <ZoomOut size={16} />
            </button>
