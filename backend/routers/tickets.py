...
async def create_ticket(ticket_data: TicketCreate, current_user: dict = Depends(get_current_user)):
    ...
    ticket = Ticket(
        ...
        status="INTERNAL", # UPDATED DEFAULT STATUS
        ...
        region=site_region if ticket_data.site_id else None # DERIVED FROM SITE
    )
...
async def get_tickets(
    ...
    if assigned_to_division and assigned_to_division != 'all':
        if assigned_to_division == "Infra & Fiberzone":
            match_stage["assigned_to_division"] = {"$in": ["Infra", "Fiberzone"]}
        elif assigned_to_division == "TS & Apps":
            match_stage["assigned_to_division"] = {"$in": ["TS", "Apps"]}
        else:
...
async def delete_ticket(ticket_id: str, current_user: dict = Depends(get_current_user)):
    # Check permissions: Admin, SuperUser, Manager, VP can delete
    if current_user["role"] not in ["Admin", "SuperUser", "Manager", "VP"]:
        raise HTTPException(
