...
async def get_dashboard(current_user: dict = Depends(get_current_user)):
    ...
    # Use local timezone for start of week calculation
    start_of_week = today - timedelta(days=today.weekday())
    weekly_counts = []
    
    # FETCH ALL SCHEDULES FOR THE WEEK - GLOBAL
    # Get all schedules within the current week to calculate global person counts
    end_of_week = start_of_week + timedelta(days=6)
    all_weekly_schedules = await db.schedules.find({
        # Any schedule that overlaps with this week
    ...
    # Global unique persons today count
