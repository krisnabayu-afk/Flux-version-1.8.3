...
async def create_report(
    ...
    # Department Mapping
    if current_user.get("department") == "Technical Operation":
    ...
    if current_user["role"] == "Staff":
        # Staff -> Needs SPV Approval
        status = "Pending SPV"
        # Division Mapping for search
        search_divisions = [target_division]
        # Ensure we look in BOTH original and mapped for better flexibility, but prioritize mapped
        ...
        # 1. Try to find SPV in same Division and Region
        query = {
            "role": "SPV", 
        ...
        spv = await db.users.find_one(query, {"_id": 0})
        # 2. Fallback: Try to find SPV in same Division without region (Global SPV)
        if not spv and target_region:
            del query["region"]
            query["region"] = {"$in": [None, "", "Global", "All Regions"]}
            spv = await db.users.find_one(query, {"_id": 0})
        # 3. Final Fallback: Any approved SPV in that division
        if not spv:
            if "region" in query: del query["region"]
            spv = await db.users.find_one(query, {"_id": 0})
            
        if not spv:
             # If no SPV found, bypass to Manager directly? 
             # OR strictly require SPV. Requirement says "Approval line like before", which usually had both.
             # We'll try to find a Manager as fallback if NO SPV exists, but set status to Pending Manager instead.
             ...
             if not manager:
                 error_msg = f"No SPV or Manager found for division {target_division}"
            ...
            current_approver = spv["id"]
            
    elif current_user["role"] == "SPV":
        # SPV -> Needs Manager Approval
        status = "Pending Manager"
        # Division Mapping for search
        search_divisions = [target_division]
        ...
        manager = await db.users.find_one(query, {"_id": 0})
        if not manager:
             # Fallback: Regionless Manager
             query["region"] = {"$in": [None, "", "Global", "All Regions"]}
             manager = await db.users.find_one(query, {"_id": 0})
        
        if not manager:
             raise HTTPException(status_code=400, detail=f"No Manager found for division {target_division}")
        current_approver = manager["id"]
...
async def get_reports(
    ...
    if approving:
        ...
        if user_role == "VP":
            ...
            # REGIONAL: Removed strict site_region match here as per user request to use staff-based region instead
            ...
            # SPV sees reports in their division/region/department that are Pending SPV
            match_stage["status"] = "Pending SPV"
            # REGIONAL: Removed strict site_region match here as per user request to use staff-based region instead
            if current_user.get("department"):
                match_stage["department"] = current_user["department"]
            # Linear Division mapping flag (SPVs also handle mapped divisions)
            manager_approving_logic = True
    ...
    if needs_lookup:
        # 0. Ensure submitted_by is a string (handles both String and ObjectId in legacy data)
        pipeline.append({
            "$addFields": {
        ...
        if division and division != "all":
            ...
            if division == "Infra & Fiberzone":
                ...
                div_list = ["TS", "Apps"]
            elif division == "Monitoring": # Added robustness for Monitoring if needed
                div_list = ["Monitoring"]
            else:
                div_list = [division]
            
            # Match against user info (requires lookup to succeed)
            # Add sensitivity for various data formats
            pipeline.append({
                "$match": {
                    "$or": [
                        {"submitter_info.division": {"$in": div_list}},
                        # Fallback for reports that might have division stored legacy (if any exist)
        ...
        if manager_approving_logic:
            ...
            # Final Approval Search Match (Division + Region)
            approval_match = {"mapped_submitter_div": current_user.get("division")}
            user_region = current_user.get("region")
            # If manager has a specific region (not Global), strictly filter by staff's region
...
async def approve_report(approval: ApprovalAction, current_user: dict = Depends(get_current_user)):
    ...
    # Determine Region for Context & Routing (Prioritize Site Region)
    report_region = site.get("region") if site else submitter.get("region") if submitter else report.get("submitted_by_region")
    
    # Determine Region for Authorization (User Request: prioritized Submitter Region)
    # This allows a Manager to approve any site region as long as the staff is in their region.
    ...
    if report_dept == "Technical Operation":
    ...
    if current_user["role"] == "VP":
        ...
        if current_user.get("department") and report_dept and current_user.get("department") != report_dept:
    ...
    if report["status"] == "Pending SPV":
        ...
        # Division Mapping for search (strictly follow Apps/TS -> TS, Fiberzone/Infra -> Infra)
        search_divisions = [submitter.get("division")]
        if submitter.get("department") == "Technical Operation":
            if submitter.get("division") in ["Apps", "TS"]:
                search_divisions = ["TS"]
            elif submitter.get("division") in ["Fiberzone", "Infra"]:
                search_divisions = ["Infra"]
        # 1. Try to find Manager in same Division and Region (Use auth_region/Staff Region as per request)
        query = {
            "role": "Manager", 
            "division": {"$in": search_divisions},
            "region": auth_region,
        ...
        if managers:
            ...
            raise HTTPException(status_code=400, detail=f"Cannot proceed: No Manager found for {search_divisions} in region {auth_region}")
...
async def cancel_report_approval(request: CancelApprovalRequest, current_user: dict = Depends(get_current_user)):
    ...
    submitter = await db.users.find_one({"id": report["submitted_by"]}, {"_id": 0})
    site = await db.sites.find_one({"id": report.get("site_id")}, {"_id": 0})
    # Determine Region for Context & Routing (Prioritize Staff Region as per Fix)
    auth_region = submitter.get("region") if submitter else report.get("submitted_by_region")
    ...
    if current_user["role"] == "VP":
        # VP can cancel any approval in their department
        if is_global_user or (current_user.get("department") and current_user.get("department") == report_dept):
            is_authorized = True
    elif current_user["role"] == "Manager":
        # Match Region (or Global) and Division
        if (is_global_user or user_region == auth_region) and current_user.get("division") == report_division:
            # Can cancel stages beyond their own
            if current_status in ["Pending VP", "Final"]:
                is_authorized = True
    elif current_user["role"] == "SPV":
        # Match Region (or Global) and Division
        if (is_global_user or user_region == auth_region) and current_user.get("division") == report_division:
            # Can cancel Manager stage
    ...
    if current_status == "Final":
        ...
        new_status = "Pending Manager"
        # Find Manager (Staff Region, Mapped Division)
        manager_query = {
            "role": "Manager",
        ...
        # Revert to Pending SPV
        new_status = "Pending SPV"
        # Find SPV (Staff Region, Mapped/Original Division)
        spv_query = {
            "role": "SPV",
            "division": report_division,
            "account_status": "approved"
        ...
        if auth_region:
            spv_query["region"] = auth_region
            
        spv = await db.users.find_one(spv_query, {"_id": 0})
        if spv:
            new_approver = spv["id"]
        else:
             # If no SPV, we might need to stay at Pending Manager?
             # But if they are cancelling, they probably want it back.
             raise HTTPException(status_code=400, detail=f"Cannot revert: No SPV found for {report_division} in region {auth_region}")
